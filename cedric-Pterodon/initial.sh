#!/bin/bash
# Sync
telegram -M "Pterodon - Moto G5: Sync started"
SYNC_START=$(date +"%s")
...
repo sync
git clone https://github.com/YuMi-Project/android_device_motorola_cedric -b android-7.1-pterodon device/motorola/cedric

SYNC_END=$(date +"%s")
SYNC_DIFF=$((SYNC_END - SYNC_START))
telegram -M "Pterodon - Moto G5: Sync completed successfully in $((SYNC_DIFF / 60)) minute(s) and $((SYNC_DIFF % 60)) seconds"
