#!/bin/bash
# Sync
telegram -M "OrangeFox R11.0 - Moto G (5S) Plus (sanders): Sync started"
SYNC_START=$(date +"%s")
...
git clone --depth 1 https://github.com/YuMi-Project/android_device_motorola_sanders.git -b fox-9.0 device/motorola/sanders
rm -rf .repo .git

SYNC_END=$(date +"%s")
SYNC_DIFF=$((SYNC_END - SYNC_START))
telegram -M "OrangeFox R11.0 - Moto G (5S) Plus (sanders): Sync completed successfully in $((SYNC_DIFF / 60)) minute(s) and $((SYNC_DIFF % 60)) seconds"

# Others
mv device/motorola/sanders/build_ofox.sh .